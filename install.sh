...
function show_animated_logo() {
    ...
    type_text_colored "\033[1;32m" "████╗░████║ ██║ ██╔══██╗ ╚════██║ ██╔══██╗ ██╔══██╗ ██╔══██╗ ██╔══██╗" 0.0001
    type_text_colored "\033[1;37m" "██╔████╔██║ ██║ ██████╔╝ ░░███╔═╝ ███████║ ██████╔╝ ██████╔╝ ██║░░██║" 0.0001
    type_text_colored "\033[1;37m" "██║╚██╔╝██║ ██║ ██╔══██╗ ██╔══╝░░ ██╔══██║ ██╔═══╝░ ██╔══██╗ ██║░░██║" 0.0001
...
function show_menu() {
    ...
    print_menu_option "12)" "Exit" "1;31"
    type_text_colored "\033[1;32m" "╚════════════════════════════════════════════════╝" 0.0005
    echo ""
    read -p "$(echo -e '\033[1;33m❯\033[0m Select an option [1-12]: ')" option
    case $option in
        1) install_bot ;;
        2) update_bot ;;
        3) remove_bot ;;
...
function install_bot() {
    ...
    ZIP_URL=$(curl -s https://api.github.com/repos/mihan-it/MirzaProBot/releases/latest | grep "zipball_url" | cut -d '"' -f 4)

if [[ "$1" == "-v" && "$2" == "beta" ]] || [[ "$1" == "-beta" ]] || [[ "$1" == "-" && "$2" == "beta" ]]; then
    ZIP_URL="https://github.com/mihan-it/MirzaProBot/archive/refs/heads/main.zip"
elif [[ "$1" == "-v" && -n "$2" ]]; then
    ZIP_URL="https://github.com/mihan-it/MirzaProBot/archive/refs/tags/v1.1.zip"
fi
...
function update_bot() {
    ...
    if [[ "$1" == "-beta" ]] || [[ "$1" == "-v" && "$2" == "beta" ]]; then
        ...
        ZIP_URL=$(curl -s https://api.github.com/repos/mihan-it/MirzaProBot/releases/latest | grep "zipball_url" | cut -d '"' -f4)
    fi
    ...
    if [ -n "$INSTALL_SCRIPT_PATH" ]; then
        ...
        RAW_INSTALL_URL="https://raw.githubusercontent.com/mihan-it/MirzaProBot/main/install.sh"
...
EOF"
    ...
    git clone https://github.com/mihan-it/MirzaProBot.git "$BOT_DIR" || {
...
function update_additional_bot() {
    ...
    if ! git clone https://github.com/mihan-it/MirzaProBot.git "$BOT_PATH"; then
